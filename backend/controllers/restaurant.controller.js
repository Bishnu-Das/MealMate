  ...
  try {
    ...
    if (status === 'ready_for_pickup') {
      ...
      if (deliveryDetailsResult.rows.length > 0) {
            ...
            const io = getIO();
            console.log(`Emitting 'new_delivery' to 'riders' room with data:`, deliveryDetails);
            io.to('riders').emit('new_delivery', deliveryDetails);

            // Store notification for all available riders
            const availableRiders = await client.query('SELECT user_id FROM rider_profiles WHERE is_available = true');
            for (const rider of availableRiders.rows) {
              await client.query(
                ...
                [order.user_id, 'rider', rider.user_id, orderId, 'delivery_status', `A new delivery is available from ${deliveryDetails.restaurant_name}.`]
              );
            }
        } else {
        console.error(`Could not fetch delivery details for order ${orderId}. Notification to riders not sent.`);
      }
    }

...
const CheckoutPage = () => {
  ...
  const totalItemsSubtotal = Object.values(ordersByRestaurant).reduce((sum, order) => sum + order.subtotal, 0);
  const totalDeliveryFee = Object.keys(ordersByRestaurant).length * deliveryFeePerRestaurant;
          ...
          {Object.keys(ordersByRestaurant).length > 0 ? (
            Object.entries(ordersByRestaurant).map(([restId, order]) => (
              <div key={restId} className="bg-white p-6 rounded-lg shadow-md">
                <h3 className="text-xl font-semibold mb-4 text-pink-600">{order.restaurant_name}</h3>
                <div className="space-y-3">
                  {order.items.map((item) => (
                    <div key={item.cart_item_id} className="flex justify-between items-center text-sm">
                      <span className="text-gray-700">{item.menu_name} x {item.quantity}</span>
